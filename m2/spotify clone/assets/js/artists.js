...
async function fetchArtists() {
    const believerData = await new DataObject(albumsUrl + "33pt9HBdGlAbRGBHQgsZsU", apiKey).fetchData();
    const believerImg = document.querySelector("#song-info img");
    believerImg.src = believerData.images[0].url;

    const artist = await new DataObject(albumsUrl + idArtist, apiKey).fetchData();
