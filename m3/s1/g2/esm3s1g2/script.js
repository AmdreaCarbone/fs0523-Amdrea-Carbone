var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var Account = /** @class */ (function () {
    function Account(saldo) {
        this.saldo = 0;
        this.saldo = saldo;
    }
    Account.prototype.deposito = function (s) {
        this.saldo += s;
        console.log("il tuo saldo ora è di " + this.saldo);
    };
    Account.prototype.prelievo = function (s) {
        this.saldo -= s;
        console.log("il saldo rimanente dopo il prelievo di " + s + " è di " + this.saldo);
    };
    return Account;
}());
var sonAccount = new Account(1000);
sonAccount.deposito(100);
sonAccount.prelievo(35);
console.log("il saldo contabile è di " + sonAccount.saldo);
var MotherAccount = /** @class */ (function (_super) {
    __extends(MotherAccount, _super);
    function MotherAccount() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    MotherAccount.prototype.addInterest = function () {
        this.saldo *= 1.1;
    };
    return MotherAccount;
}(Account));
var motherAccount = new MotherAccount(1500);
motherAccount.deposito(100);
motherAccount.prelievo(35);
motherAccount.addInterest();
console.log(motherAccount.saldo);
